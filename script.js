...
class GameController {
  constructor () {
    ...
    document.addEventListener('judged', (e) => this.record.update(e.detail.judge));
    document.addEventListener('render', (e) => this.onRender(e));

    // 繧ｦ繧｣繝ｳ繝峨え縺ｮ繝輔か繝ｼ繧ｫ繧ｹ縺悟､悶ｌ縺滓凾縺ｫ蜈ｨ菴薙ｒ繝昴�繧ｺ縺吶ｋ
  ...
  async play (mode) {
    ...
    switch (this.mode) {
      case 'easy':
        this.notesDataList = new NotesDataList(await this.easyNotesData(), this.mode);
        this.lyrics = new LyricsEasy();
        break;
      case 'normal':
        this.notesDataList = new NotesDataList(await this.normalNotesData(), this.mode);
        this.lyrics = new LyricsNormal();
        break;
      case 'hard':
        this.notesDataList = new NotesDataList(await this.hardNotesData(), this.mode);
  ...
  startRender () {
    this.currentTime = 0; // deltaTime邂怜�逕ｨ
    this.startTime = 0; // render縺ｮ邨碁℃譎る俣
    this.playTime = 0; // 譖ｲ縺ｮ蜀咲函譎る俣
    this.count = 0; // 諡肴焚
    this.playing = true;
    this.ended = false;
    this.rendering = true;
  ...
  render (timestamp) {
    const now = (timestamp || 0) * 0.001; // 遘偵↓螟画鋤
    ...
    if (this.currentTime == 0) {
      ...
      document.querySelectorAll('.note').forEach((el) => el.dispatchEvent(event));
...
class ViewController {
  constructor (gameController) {
    ...
    document.querySelector('.btn-play-easy').addEventListener('click', () => {
      this.btnAction(() => {
        ...
        if (typeof this.onClickPlayEasyBtn == 'function') this.onClickPlayEasyBtn();
      });
    });
    document.querySelector('.btn-play-normal').addEventListener('click', () => {
      this.btnAction(() => {
        ...
        if (typeof this.onClickPlayNormalBtn == 'function') this.onClickPlayNormalBtn();
      });
    });
    document.querySelector('.btn-play-hard').addEventListener('click', () => {
      this.btnAction(() => {
        ...
        if (typeof this.onClickPlayHardBtn == 'function') this.onClickPlayHardBtn();
      });
    });
    ...
    document.addEventListener('keydown', (e) => {
      switch (e.keyCode) {
        case 65: { this.judgeEvent(1); break; } // a
        case 83: { this.judgeEvent(2); break; } // s
        case 68: { this.judgeEvent(3); break; } // d
        case 70: { this.judgeEvent(4); break; } // f
        case 71: { this.judgeEvent(5); break; } // g
      }
    });
    document.querySelectorAll('.btn-judge > *').forEach((el, index) => new JudgeBtn(el, index));
  }
  // 騾｣邯壹〒蜃ｦ逅�′螳溯｡後＆繧後↑縺�ｈ縺�↓縺吶ｋ
  btnAction (callback) {
  ...
  judgeEvent (number) {
    ...
    document.querySelectorAll(`.lane${number} .note`).forEach((el) => el.dispatchEvent(event));
  }
}
class View {
  get showed () {
  ...
  show () {
    ...
    document.querySelectorAll('.view').forEach((el) => el.classList.remove('over-layer', 'under-layer'));
    if (this.el) this.el.classList.add('over-layer');
    if (outEl) outEl.classList.add('under-layer');

    let event = new CustomEvent('showView', {detail: {name: this.constructor.name}});
    document.dispatchEvent(event);
  }
}
...
class SelectView extends View {
  ...
  showBtn (mode) {
    switch (mode) {
      case 'easy': { this.btnPlayEasyEl.style.display = 'block'; break; }
      case 'normal': { this.btnPlayNormalEl.style.display = 'block'; break; }
      case 'hard': { this.btnPlayHardEl.style.display = 'block'; break; }
    }
  }
  hideBtn (mode) {
    switch (mode) {
      case 'easy': { this.btnPlayEasyEl.style.display = 'none'; break; }
...
class PlayView extends View {
  ...
  constructor () {
    ...
    this.btnJudgeEls = this.el.querySelectorAll('.btn-judge > *');
    this.reloadDialogEl = this.el.querySelector('.dialog-reload');
  ...
  reset () {
    ...
    document.querySelectorAll('.btn-judge > *').forEach((el) => el.innerHTML = '');
    this.hideEndText();
  }
  showEndText () {
  ...
  updateScore (score) {
    let html = (score + '').split('').map((value) => `<div data-number="${value}"></div>`).join('');
  ...
  updateCombo (combo) {
    if (combo < COMBO_DISPLAY_COUNT) {
      ...
      let html = (combo + '').split('').map((value) => `<div data-number="${value}"></div>`).join('');
...
class ResultView extends View {
  ...
  updateScore (value) {
    let html = (value + '').split('').map((value) => `<div data-number="${value}"></div>`).join('');
...
class JudgeBtn {
  ...
  onClick () {
    ...
    document.querySelectorAll(`.lane${this.index + 1} .note`).forEach((el) => el.dispatchEvent(event));
  }
}
class NotesDataList {
  get waitTime () {
  ...
  convert () {
    let currentTime = 0; // 蜃ｦ逅�ｒ螳溯｡後☆繧区凾髢�
      ...
      data.lanes.forEach((row, j) => {
        ...
        row.forEach((value, k) => {
          if (k < 5 && value == 1) {
            ...
            if (i == 0 && j < beatCount) {
              note.top += judgeData.distance * (1 - (j / (judgeData.moveToBeatCount))) - judgeData.distance / judgeData.speedRate;
            }
...
class Note {
  ...
  // @param [Number] index 繝ｬ繝ｼ繝ｳ縺ｮ繧､繝ｳ繝�ャ繧ｯ繧ｹ
  ...
  judgement () {
    ...
    if (this.judgeRange.miss.max >= y) {
      if (this.judgeRange.perfect.min <= y && this.judgeRange.perfect.max >= y) {
        this._judgement = 'perfect';
      } else if (this.judgeRange.good.min <= y && this.judgeRange.good.max >= y) {
  ...
  fireEvent () {
    let event  = new CustomEvent('judged', {detail: {index: this.index, judge: this.judgement()}});
    document.dispatchEvent(event);
  }
}
// 繧ｹ繧ｳ繧｢繧�さ繝ｳ繝懊ｒ邂｡逅�
class RecordData {
  ...
  // 蜷�ｨｮ謨ｰ蛟､縺梧峩譁ｰ縺輔ｌ縺滓凾縺ｫ繧､繝吶Φ繝医ｒ逋ｺ轣ｫ
  fireEvent (name, value) {
    let event = new CustomEvent('updateRecord', {detail: {name: name, value: value}});
    document.dispatchEvent(event);
  }
}
// 蛻､螳壹ｄ繝弱�繝��蜍輔″縺ｫ蠢�ｦ√↑蜷�ｨｮ謨ｰ蛟､繧堤ｮ｡逅�
class JudgeData {
  constructor (bpm, mode) {
    ...
    // 繝弱�繝�′蛻､螳壹Λ繧､繝ｳ縺ｫ遘ｻ蜍輔☆繧九∪縺ｧ縺ｮ諡肴焚
    this.moveToBeatCount = BEAT_PER_BAR * this.speedRate;
    // 譛蠕後�繝弱�繝�ｒ逕滓�縺励※縺九ｉ縲梧ｼ泌･冗ｵゆｺ�阪ｒ陦ｨ遉ｺ縺吶ｋ縺ｾ縺ｧ縺ｮ譎る俣�育ｧ抵ｼ�
  ...
  showJudgeArea () {
    document.querySelector('.judge-area-perfect').style.top = `${this.judgeRange.perfect.min}px`;
    document.querySelector('.judge-area-perfect').style.height = `${this.judgeRange.perfect.max - this.judgeRange.perfect.min}px`;
    document.querySelector('.judge-area-good').style.top = `${this.judgeRange.good.min}px`;
    document.querySelector('.judge-area-good').style.height = `${this.judgeRange.good.max - this.judgeRange.good.min}px`;
    document.querySelector('.judge-area-miss').style.top = `${this.judgeRange.miss.min}px`;
    document.querySelector('.judge-area-miss').style.height = `${this.judgeRange.miss.max - this.judgeRange.miss.min}px`;
  }
}
class Sound {
  get duration () {
  ...
  // @param [String] src
  // @param [Object] options loop,volume縺ｮ縺ｿ
  constructor (src, options = {}) {
    ...
    (async () => {
      ...
      this.context.decodeAudioData(responseBuffer, (buffer) => this.buffer = buffer);
    })();
  }
  prepare () {
    // source -> gainNode -> context 縺ｮ鬆�↓謗･邯�
    ...
    if (typeof this.options.volume == 'number') this.volume(this.options.volume);
...
function init () {
  ...
  var imgAspect = 1920 / 1080; // 閭梧勹逕ｻ蜒上�繧｢繧ｹ繝壹け繝�
  var resizeAction = () => {
